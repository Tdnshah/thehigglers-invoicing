<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="gst_number" name="gst_number" type="text" class="mt-1 block w-full uppercase" :value="old('gst_number', $user->gst_number)" placeholder="22AAAAA0000A1Z5" />
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div>
                <x-input-label for="bank_name" :value="__('Bank Name (Optional)')" />
                <x-text-input id="bank_name" name="bank_name" type="text" class="mt-1 block w-full" :value="old('bank_name', $user->bank_name)" />
                <x-input-error class="mt-2" :messages="$errors->get('bank_name')" />
            </div>

            <div>
                <x-input-label for="bank_ifsc" :value="__('IFSC Code (Optional)')" />
                <x-text-input id="bank_ifsc" name="bank_ifsc" type="text" class="mt-1 block w-full uppercase" :value="old('bank_ifsc', $user->bank_ifsc)" />
                <x-input-error class="mt-2" :messages="$errors->get('bank_ifsc')" />
            </div>
        </div>

        <div>
            <x-input-label for="bank_account_number" :value="__('Account Number (Optional)')" />

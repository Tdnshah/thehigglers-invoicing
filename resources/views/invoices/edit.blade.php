...
<x-app-layout>
                    ...
                    <form method="POST" action="{{ route('invoices.update', $invoice) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="due_date" class="block mt-1 w-full" type="date" name="due_date" :value="old('due_date', optional($invoice->due_date)->format('Y-m-d'))" />
                            ...
                            <div>
                                ...
                                <select id="invoice_type" name="invoice_type" x-model="invoiceType" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                    <option value="regular" {{ old('invoice_type', $invoice->invoice_type) == 'regular' ? 'selected' : '' }}>Regular</option>
                                    <option value="export" {{ old('invoice_type', $invoice->invoice_type) == 'export' ? 'selected' : '' }}>Export</option>
                                    <option value="interstate" {{ old('invoice_type', $invoice->invoice_type) == 'interstate' ? 'selected' : '' }}>Interstate</option>
                                </select>
                        ...
                        @if(isset($companyCustomFields) && $companyCustomFields->isNotEmpty())
                        @php
                            $__cfExisting = collect(old('custom_fields', $invoice->custom_fields ?? []))
                        ...
                        <div class="mb-6"
                            ...
                            <p x-show="selected.length === 0" class="text-sm text-gray-400 italic mb-2">
                                Click "+ Add Field" to attach a custom field to this invoice.
                            </p>
    ...
    <script>
        function invoiceForm() {
            return {
                invoiceType: "{{ old('invoice_type', $invoice->invoice_type ?? 'regular') }}",
                currency: "{{ old('currency', $invoice->currency ?? 'INR') }}",

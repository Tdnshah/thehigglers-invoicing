...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .invoice-title { display: table-cell; vertical-align: top; text-align: right; width: 40%; }
            
            .parties-row { display: table; width: 100%; margin-top: 20px; }
            .party-box { display: table-cell; vertical-align: top; width: 48%; }
            
            .invoice-details-grid { display: block; }
            .invoice-details-grid div { margin-bottom: 5px; }
        }
        .header-row {
            display: flex;
        ...
        .invoice-title h2 { margin: 0; font-size: 20px; text-transform: uppercase; color: #666; }
...
<body>
    ...
    <div class="no-print" style="text-align: center; margin-bottom: 20px; padding: 10px; background: #f3f4f6; border-bottom: 1px solid #d1d5db;">
        ...
        <a href="{{ route('invoices.download', $invoice) }}" style="background: #4b5563; color: white; border: none; padding: 10px 20px; border-radius: 4px; cursor: pointer; font-size: 14px; text-decoration: none; margin-left: 10px;">Download PDF</a>
    ...
    <div class="container">
        ...
        <div class="header-row">
            <div class="company-details">
                @if($invoice->user->company && $invoice->user->company->logo_path)
                    <?php 
                        $path = storage_path('app/public/' . $invoice->user->company->logo_path);
                    ...
                    ?>
                @endif
                <h1>{{ $invoice->user->company->name ?? 'Company Name' }}</h1>
                @if($invoice->user->company)
                    <div>{{ $invoice->user->company->address }}</div>
                    <div>Email: {{ $invoice->user->email }}</div>
                    <div class="gst-label">GSTIN: <strong>{{ $invoice->user->company->gst_number }}</strong></div>
                @endif
            </div>
            <div class="invoice-title">
                ...
                <div style="margin-top: 10px; font-size: 12px;">
                    @if($invoice->invoice_type === 'export')
                        <span class="badge">Export Invoice</span>
                        @if($invoice->lut_number)
                            <br><span style="font-size: 10px;">LUT: {{ $invoice->lut_number }}</span>
                        @endif
                    @elseif($invoice->invoice_type === 'interstate')
        ...
        <div class="parties-row">
            <div class="party-box">
                ...
                <div class="gst-label" style="margin-top: 5px;">GSTIN: <strong>{{ $invoice->client->gst_number ?? 'N/A' }}</strong></div>
            </div>
            <div class="party-box" style="text-align: right;">
                <div class="box-title" style="text-align: right;">Invoice Details</div>
                <div class="invoice-details-grid" style="justify-content: end;">
                    ...
                    <div>{{ $invoice->place_of_supply ?? ($invoice->client->gst_number ? substr($invoice->client->gst_number, 0, 2) : 'N/A') }}</div>
        ...
        <div style="display: flex; justify-content: space-between; margin-top: 20px;">
            ...
            <div style="width: 50%; padding-right: 20px;">
                @if($invoice->user->company && $invoice->user->company->bank_account_number)
                <div style="border: 1px solid #eee; padding: 15px; border-radius: 4px; background: #f9fafb;">
                    ...
                    <table style="width: 100%; margin: 0; font-size: 12px;">
                        <tr>
                            ...
                            <td style="padding: 2px 0; border: none; font-weight: bold;">{{ $invoice->user->company->bank_name }}</td>
                        </tr>
                        <tr>
                            <td style="padding: 2px 0; border: none; width: 100px; color: #666;">Account No:</td>
                            <td style="padding: 2px 0; border: none; font-weight: bold;">{{ $invoice->user->company->bank_account_number }}</td>
                        </tr>
                        <tr>
                            <td style="padding: 2px 0; border: none; width: 100px; color: #666;">IFSC Code:</td>
                            <td style="padding: 2px 0; border: none; font-weight: bold;">{{ $invoice->user->company->bank_ifsc }}</td>
                        </tr>
                    </table>
                </div>
                @endif
                ...
                <table class="totals-table" style="width: 100%;">
                    ...
                    <tr>
                        <td colspan="2" style="font-size: 10px; color: #666; font-style: italic; padding-top: 10px; text-align: right;">
                             Amount in words:<br>
                             {{ Number::spell($invoice->total) }} {{ $invoice->currency }} Only
                        </td>
                    </tr>
                </table>
            </div>
